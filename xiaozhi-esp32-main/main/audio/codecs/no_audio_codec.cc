...
int NoAudioCodec::Read(int16_t* dest, int samples) {
    ...
    for (int i = 0; i < samples; i++) {
        ...
        dest[i] = (value > INT16_MAX) ? INT16_MAX : (value < INT16_MIN) ? INT16_MIN : static_cast<int16_t>(value);
        int32_t magnitude = std::abs(static_cast<int32_t>(dest[i]));
        sum_squares += static_cast<int64_t>(magnitude) * magnitude;
        if (magnitude > peak) {
            peak = magnitude;
        }
    }

    static int64_t diagnostic_samples = 0;
    static int64_t diagnostic_squares = 0;
    static int32_t diagnostic_peak = 0;
    static int64_t last_diagnostic_ms = 0;
    diagnostic_samples += samples;
    diagnostic_squares += sum_squares;
    if (peak > diagnostic_peak) {
        diagnostic_peak = peak;
    }
    int64_t now_ms = esp_timer_get_time() / 1000;
    if (now_ms - last_diagnostic_ms >= 1000) {
        int32_t rms = diagnostic_samples > 0
            ? static_cast<int32_t>(std::sqrt(static_cast<double>(diagnostic_squares) / diagnostic_samples))
            : 0;
        if (rms < 1200) {
            ESP_LOGW(TAG, "Mic level low: RMS=%ld Peak=%ld", static_cast<long>(rms), static_cast<long>(diagnostic_peak));
        } else if (diagnostic_peak > 30000) {
            ESP_LOGW(TAG, "Mic clipping risk: RMS=%ld Peak=%ld", static_cast<long>(rms), static_cast<long>(diagnostic_peak));
        } else {
            ESP_LOGI(TAG, "Mic level: RMS=%ld Peak=%ld", static_cast<long>(rms), static_cast<long>(diagnostic_peak));
        }
        diagnostic_samples = 0;
        diagnostic_squares = 0;
        diagnostic_peak = 0;
