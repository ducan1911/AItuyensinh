...
#include "board.h"
#include "assets/school_logo.h"
extern "C" {
extern const lv_image_dsc_t logo;
extern const lv_image_dsc_t mic;
extern const uint8_t _binary_songamthanh_gif_start[];
extern const uint8_t _binary_songamthanh_gif_end[];
}
...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_t* school_name_label = lv_label_create(screen);
    lv_label_set_text(school_name_label, "TRUONG TC KINH TE - KY THUAT\nTP. CAN THO");
    lv_obj_set_width(school_name_label, LV_HOR_RES);
    lv_obj_set_style_text_align(school_name_label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_color(school_name_label, lv_color_hex(0x124B82), 0);
    lv_obj_align(school_name_label, LV_ALIGN_TOP_MID, 0, 32);

    lv_obj_t* title_label = lv_label_create(screen);
    lv_label_set_text(title_label, "AI TRO LY TUYEN SINH");
    lv_obj_set_width(title_label, LV_HOR_RES - 16);
    lv_obj_set_style_text_align(title_label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_color(title_label, lv_color_hex(0x16804B), 0);
    lv_obj_align(title_label, LV_ALIGN_TOP_MID, 0, 78);

    school_logo_image_ = lv_image_create(screen);
    lv_image_set_src(school_logo_image_, &logo);
    lv_image_set_scale(school_logo_image_, 238);
    lv_obj_align(school_logo_image_, LV_ALIGN_CENTER, 0, 14);

    waveform_ = lv_image_create(screen);
    lv_obj_set_size(waveform_, 120, 25);
    lv_obj_align(waveform_, LV_ALIGN_BOTTOM_MID, 0, -18);
    lv_obj_set_style_opa(waveform_, LV_OPA_COVER, 0);
    lv_obj_add_flag(waveform_, LV_OBJ_FLAG_HIDDEN);
    static lv_img_dsc_t waveform_dsc = {};
    waveform_dsc.header.magic = LV_IMAGE_HEADER_MAGIC;
    waveform_dsc.header.cf = LV_COLOR_FORMAT_RAW;
    waveform_dsc.data = _binary_songamthanh_gif_start;
    waveform_dsc.data_size = _binary_songamthanh_gif_end - _binary_songamthanh_gif_start;
    waveform_gif_controller_ = std::make_unique<LvglGif>(&waveform_dsc);
    if (waveform_gif_controller_->IsLoaded()) {
        waveform_gif_controller_->SetFrameCallback([this]() {
            lv_image_set_src(waveform_, waveform_gif_controller_->image_dsc());
        });
        lv_image_set_src(waveform_, waveform_gif_controller_->image_dsc());
    }
    ...
    status_label_ = lv_label_create(status_bar_);
    lv_obj_set_width(status_label_, LV_HOR_RES * 0.85);
    lv_label_set_long_mode(status_label_, LV_LABEL_LONG_SCROLL_CIRCULAR);
    lv_obj_set_style_text_align(status_label_, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_color(status_label_, lv_color_hex(0x16804B), 0);
    lv_label_set_text(status_label_, "Chờ nói.....");
    lv_obj_align(status_label_, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_y(status_bar_, 240);
    ...
    lv_label_set_text(chat_message_label_, "");
    lv_obj_set_width(chat_message_label_, LV_HOR_RES - lvgl_theme->spacing(8));
    lv_obj_set_height(chat_message_label_, text_font->line_height);
...
void LcdDisplay::SetStatus(const char* status) {
    ...
    DisplayLockGuard lock(this);
    if (status != nullptr && (strstr(status, "nghe") != nullptr || strstr(status, "Nghe") != nullptr)) {
        lv_obj_remove_flag(waveform_, LV_OBJ_FLAG_HIDDEN);
        if (waveform_gif_controller_ && waveform_gif_controller_->IsLoaded()) {
            waveform_gif_controller_->Start();
        }
    } else {
        lv_obj_add_flag(waveform_, LV_OBJ_FLAG_HIDDEN);
        if (waveform_gif_controller_) {
            waveform_gif_controller_->Stop();
        }
    }
}
...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    if (image->IsGif()) {
        ...
        lv_obj_add_flag(emoji_label_, LV_OBJ_FLAG_HIDDEN);
        if (school_logo_image_ != nullptr) {
            lv_obj_add_flag(school_logo_image_, LV_OBJ_FLAG_HIDDEN);
        }
