...
#define QUESTION_LOG_URL "https://aituyensinh-bridge.onrender.com/webhook/log"

extern const uint8_t _binary_gtsr4_pem_start[] asm("_binary_gtsr4_pem_start");
...
static void LogConversationTask(void* arg) {
    auto* conversation = static_cast<ConversationLog*>(arg);
    cJSON* root = cJSON_CreateObject();
    cJSON_AddStringToObject(root, "question", conversation->question.c_str());
    cJSON_AddStringToObject(root, "answer", conversation->answer.c_str());
    char* payload = cJSON_PrintUnformatted(root);
    esp_http_client_config_t config = {};
    config.url = QUESTION_LOG_URL;
    config.method = HTTP_METHOD_POST;
    config.timeout_ms = 30000;
    config.cert_pem = reinterpret_cast<const char*>(_binary_gtsr4_pem_start);
    ...
    if (client != nullptr) {
        ...
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "Conversation log failed: %s", esp_err_to_name(err));
        } else {
            ESP_LOGI(TAG, "Conversation log sent to Render");
        }
...
static void LogConversationToBridge(const std::string& question, const std::string& answer) {
    if (question.empty()) {
        return;
    }
    auto* conversation = new ConversationLog{question, answer};
    if (xTaskCreate(LogConversationTask, "conversation_log", 8192, conversation, 3, nullptr) != pdPASS) {
        delete conversation;
        ESP_LOGW(TAG, "Could not create conversation log task");
    }
}
    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                ...
                LogConversationToBridge(pending_question_, pending_answer_);
                ...
                if (cJSON_IsString(text)) {
                    ...
                    pending_answer_ += text->valuestring;
            ...
            if (cJSON_IsString(text)) {
                ESP_LOGI(TAG, ">> %s", text->valuestring);
                pending_question_ = text->valuestring;
                pending_answer_.clear();
                Schedule([display, message = std::string(text->valuestring)]() {
