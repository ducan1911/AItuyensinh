...
### 3. Phan mem - Chua lam
- [ ] Deploy `dify_bridge.py` len Railway (hoac cloud khac) de khong can chay tren laptop
- [ ] Cai dat va build firmware Xiaozhi len ESP32-S3
- [ ] Cau hinh Xiaozhi tro den URL cua dify_bridge (tren cloud)
...
### A. Khong can PCM5102 — lam ngay duoc:
1. **Deploy `dify_bridge.py` len Railway:**
   ...
   - Se co URL dang: `https://xxx.up.railway.app`

2. **Test dify_bridge tren cloud:**
   - Gui curl test: `curl -X POST https://xxx.up.railway.app/v1/chat/completions -H "Content-Type: application/json" -d @test.json`
   - Kiem tra dashboard.html co hien du lieu moi khong
...
5. **Cau hinh Xiaozhi:**
   - LLM Base URL: `https://xxx.up.railway.app/v1` (URL Railway)
...
| `dify_bridge.py` | Backend Flask proxy | DA VIET XONG, CHUA DEPLOY |
